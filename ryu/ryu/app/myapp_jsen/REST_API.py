...
class RestApi(simple_switch_13.simpleswitch13):
    ...
    def switch_feature_handler(self,ev):
        super(RestApi,self).switch_feature_handler(ev)
        ...
        if datapath is not None:
            ...
            if entry_port not in mac_table.values():
                for mac,port in mac_table.items():
                    #from konwn device to new device
                    actions = [parser.OFPAcitonOutput(entry_port)]
                    match = parser.OFPMatch(in_port=port,eth_dst=entry_mac)
                    self.add_flows(datapath,1,match,actions)

                    #from new device to known device
                    actions = [parser.OFPAcitonOutput(port)]
