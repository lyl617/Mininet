...
class LearningSwitch(object):
  ...
  def __init__(self,connection,transparent):
  	...
  	#We want to hear PackeIn message,wo we listen to the connection
  ...
  def _handle_PacketIn(self,event):
    ...
    def flood(message = None):
    	...
    	if time.time() - self.connection.connect_time >= _flood_delay:
    		...
    		if not self.hold_down_expired:
    			...
    			log.info("%s:Flood hold_down expored --flooding",dpid_to_str(event.dpid))
    ...
    if packet.dst.is_multicast:
    	...
    	if packet.dst not in self.macToPort:
    		flood("Port for %s unknown -- flooding"%(packet.dist,))
  		else:
